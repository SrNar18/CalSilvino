...
const MAX_SIZE_BYTES = 4 * 1024 * 1024; // 4 MB — base64 overhead brings this to ~5.4 MB in transit, safely under Lambda's 6 MB limit
...
export default async (req) => {
    try {
        ...
        if (pdfBuffer.length > MAX_SIZE_BYTES) {
            return Response.json({ error: 'El archivo supera los 4 MB' }, { status: 400 });
        }
